...
public abstract class HistoryDao extends BaseDao<History> {
    ...
    @Query("SELECT * FROM History WHERE cid IN (:cids) ORDER BY createTime DESC")
    public abstract List<History> find(List<Integer> cids);
    ...
    @Query("SELECT * FROM History WHERE cid IN (:cids) AND vodName = :vodName")
    public abstract List<History> findByName(List<Integer> cids, String vodName);
