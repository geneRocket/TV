...
public class EpisodeDialog extends BaseDialog implements ArrayPresenter.OnClickListener {
    ...
    private void setEpisodeView(List<Episode> items) {
        ...
        int maxHeight = ResUtil.getScreenHeight() * 3 / 4;
        int minHeight = ResUtil.getScreenHeight() / 3;
        int contentHeight = ResUtil.dp2px(24 + Math.max(rowNum, 1) * 48 + (rowNum > 1 ? (rowNum - 1) * 8 : 0));
        int height = Math.max(minHeight, Math.min(maxHeight, contentHeight));
